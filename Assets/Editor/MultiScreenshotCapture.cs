...
namespace MultiScreenshotCaptureNamespace
{
	...
	public class MultiScreenshotCapture : EditorWindow
	{
		...
		private const string SESSION_DATA_PATH = "Assets/Resources/MultiScreenshotCapture_Settings.json";
		...
		// TAKE A SCREENSHOTS FROM KEY PRESSED
		private float previousScreenshotsTime;
		...
		{
			if(Time.time - previousScreenshotsTime < 0.5f)
			{
				return;
			}
			
			previousScreenshotsTime = Time.time;
